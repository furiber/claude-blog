...
| `/blog discourse <topic> --input results.json` | Skip search; build the brief from a pre-gathered results file. The flag name matches `${CLAUDE_PLUGIN_ROOT}/skills/blog-discourse/scripts/discourse_research.py --input` directly. |
...
Before any search, run the four keyword-trap checks from `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/research-quality.md` (Class 1 demographic shopping, Class 2 numeric trap, Class 3 overly-literal phrase, Class 4 generic single-noun). If the topic matches a class:
...
Invoke `${CLAUDE_PLUGIN_ROOT}/skills/blog-discourse/scripts/discourse_research.py` to:
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/blog-discourse/scripts/discourse_research.py \
  --input "$RESULTS_JSON" \
...
Apply the 6 LAWs from `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/synthesis-contract.md`:
