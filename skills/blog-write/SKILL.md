...
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/synthesis-contract.md`: 6 LAWs for synthesis output (v1.8.0; applies whenever the article embeds research-synthesis prose)
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/content-templates.md`: Template selection guide and usage
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/quality-scoring.md`: 5-category scoring (Content 30, SEO 25, E-E-A-T 15, Technical 15, AI Citation 15)
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/eeat-signals.md`: Experience, expertise, authority, trust markers
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/internal-linking.md`: Linking strategy and anchor text rules
- `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/visual-media.md`: Image sourcing and chart styling
- `${CLAUDE_PLUGIN_ROOT}/skills/blog-write/references/delivery.md`: delivery contract steps and summary template for this sub-skill
...
`${CLAUDE_PLUGIN_ROOT}/skills/blog/references/flow-alignment.md` and `/blog flow find`.
...
`${CLAUDE_PLUGIN_ROOT}/skills/blog/templates/` (the main `blog` skill owns the templates directory).
...
2. **Load the matching template**: Check `.claude-blog/templates/<type>.md` in the working directory first; a file there is the user's own customization and wins. Otherwise read `${CLAUDE_PLUGIN_ROOT}/skills/blog/templates/<type>.md`
...
See `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/content-templates.md` for detailed selection criteria and intent mapping.
...
1. **Find 8-12 current statistics** (2025-2026 data preferred)
   ...
   - Prioritize tier 1-3 sources (see `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/quality-scoring.md`)
...
2. **Find a cover image** (wide, high-quality, topic-relevant):
   ...
   - See `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/visual-media.md` for cover image sizing details
...
4. **Plan 2-4 data visualizations** from researched statistics
   - Select diverse chart types (see `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/visual-media.md`)
...
7. **Find relevant YouTube videos** (2-3 per post):
   ...
   - Apply quality criteria from `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/video-embeds.md` (min score 50/100)
...
- See `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/cta-placement.md` for placement rules by content type
...
`${CLAUDE_PLUGIN_ROOT}/skills/blog/references/content-rules.md` Visual Rhythm section and
`${CLAUDE_PLUGIN_ROOT}/skills/blog/references/cta-placement.md` for CTA positioning.
...
See `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/visual-media.md` for chart type selection and styling rules.
...
(never "click here" or "read more"). See `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/internal-linking.md` for
...
#### 5k. Video Embedding
Embed YouTube videos using srcdoc lazy-loading pattern from `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/video-embeds.md`.
...
19. **YouTube videos** - 2-3 embeds with lazy loading, aria-labels, and noscript fallback (see `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/video-embeds.md`)

### Phase 6.5: Delivery Contract Enforcement (v1.9.0)
Before Phase 7, run the 5-gate delivery contract (via `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/blog_preflight.py` plus a BLOCKING `blog-reviewer` agent) per `${CLAUDE_PLUGIN_ROOT}/skills/blog/references/blog-delivery-contract.md` and the writer-specific checklist in `${CLAUDE_PLUGIN_ROOT}/skills/blog-write/references/delivery.md`. Use `python3` for local scripts. The user is never the first reviewer; the gates are.
...
### Phase 7: Delivery
Present the completed article only after Phase 6.5 returns all gates passing. Include `<folder>/preview/*.png` screenshots and the compact completion summary described in `${CLAUDE_PLUGIN_ROOT}/skills/blog-write/references/delivery.md`.
